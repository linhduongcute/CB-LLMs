...
if __name__ == "__main__":
    ...
    # --- Encode datasets như cũ ---
    encoded_train_dataset = train_dataset.map(
        lambda e: tokenizer(e[CFG.example_name[dataset]], padding=True, truncation=True, max_length=args.max_length),
        batched=True, batch_size=len(train_dataset)
    ).remove_columns([CFG.example_name[dataset]])
    if dataset == 'SetFit/sst2':
    ...
    if dataset == 'SetFit/sst2':
        ...
        ).remove_columns([CFG.example_name[dataset]]).remove_columns(['label_text'])
    ...
    # --- Build TensorDataset + Linear Layer + Train như cũ ---
    ...
    if dataset == 'SetFit/sst2':
        ...
        output_proj = glm_saga(linear, indexed_train_loader, STEP_SIZE, args.saga_epoch, ALPHA, k=10,
                               test_loader=test_loader, do_zero=True, n_classes=CFG.class_num[dataset])
    ...
    if dataset == 'SetFit/sst2':
        ...
        output_proj = glm_saga(linear, indexed_train_loader, STEP_SIZE, args.saga_epoch, ALPHA, epsilon=1, k=1,
                               test_loader=test_loader, do_zero=False, n_classes=CFG.class_num[dataset],
                               metadata=metadata, n_ex=train_c.shape[0])
